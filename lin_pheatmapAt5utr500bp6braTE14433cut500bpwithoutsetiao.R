...
bmp(file ="At5utr500bp6bra14433merge1sortedreplacefillupreversecut500bp.bmp",width =2000*2,height = 2000*3,units = "px",res =72*1.5)
# df<- read.table(args$input,header = FALSE,row.names = 1)
df<- read.table("At5utr500bp6bra14433merge1sortedreplacefillupreversecut500bp",header=FALSE,row.names = 1)
...
p<-pheatmap(df,clustering_method = "ward.D",cluster_cols = FALSE,color = colorRampPalette(c("black","red"))(1800),
            cellwidth =4,cellheight=0.25,show_rownames=F,show_colnames = F,legend_breaks=c(0:6),border=FALSE,
