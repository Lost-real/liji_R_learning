...
# 导入数据
plot_data1 <- read.csv(file = "00002柱状图.csv")
plot_data2 <- read.csv(file = "00001折线图.csv")
#-----------------------
p<- ggplot()+ 
  ...
  geom_signif(data=plot_data2,mapping=aes(x=group,y=Retive_Abundance), # 不同组别的显著性
              ...
              y_position = c(6,6.5,7), # 设置显著性线的位置高度
              size=1, # 修改线的粗细
              textsize = 7, # 修改*标记的大小
              test = "t.test")+ # 检验的类型
  geom_errorbar(data=plot_data1,mapping=aes(x = group,ymin = mean-sd, ymax = mean+sd), # 误差线添加
                ...
                size=0.8)+ #粗细
  scale_y_continuous(limits =c(0,8) ,expand = c(0,0))+ # y轴的范围
