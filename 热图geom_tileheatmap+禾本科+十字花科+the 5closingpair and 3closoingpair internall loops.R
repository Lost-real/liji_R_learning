...
library (ggplot2)
#library(tidyverse)
bmp(file ="十字花科5’UTR 5closing pair and 3'closing pair of internal loops-3-1.bmp",width =750*2,height = 750*2,units = "px",res =72*2)
df<- read.csv("十字花科5’UTR 5closing pair and 3'closing pair of internal loops-3-1.csv")
...
ggplot(df,aes(x,y,fill=number))+geom_tile(width=1,height=1,color="black")+
  theme_classic()+
  scale_fill_gradientn(colours=cols,limits=c(-0.5,3),breaks=seq(-0.5,3,0.5),
                       labels=c("0","10^0.0","10^0.5","10^1.0","10^1.5","10^2.0","10^2.5","10^3.0"),
                       guide=guide_colourbar(ticks=T,ticks.colour = "black", nbin=300,barheight=12, label=T,barwidth=2))+  
  scale_y_continuous(limits=c(0,17),breaks=seq(1,16,1),
                     labels=c("U:A","A:U","C:G","G:C","U:G","G:U","C:A","G:G","A:C","A:G","C:U","G:A",
                              "A:A","C:C","U:C","U:U"))+
  scale_x_continuous(limits=c(0,17),breaks=seq(1,16,1),
                     labels=c("A:U","C:G","U:A","G:C","U:G","G:U","A:C","A:G","G:A","A:A","G:G","C:A",
                              "U:U","C:U","C:C","U:C"))+
...
labs(x="5' Closing Pair",y="3' Closing Pair")+
  theme(legend.background = element_rect(fill = "grey90",colour = "grey90"),
        ...
        legend.text=element_text(colour="black",hjust=0.7,vjust=0.5,size=10,family = "serif",face="bold"),
...
library (ggplot2)
#library(tidyverse)
bmp(file ="禾本科5UTR the 5'closing pair and 3'closing pair of internal loops 3-1.bmp",width =750*2,height = 750*2,units = "px",res =72*2)
df<- read.csv("禾本科5UTR the 5'closing pair and 3'closing pair of internal loops 3-1.csv")
# df$y<-factor(df$y,levels=rev(unique(df$y)))
# df$y<-factor(df$y,levels=unique(df$y))
#df$y%>%factor()%>%fct_inorder()
#pivot_longer(df)
