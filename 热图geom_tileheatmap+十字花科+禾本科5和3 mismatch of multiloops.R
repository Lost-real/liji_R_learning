...
library (ggplot2)
#library(tidyverse)
bmp(file ="十字花科 5 and 3 mismatch of multiloops2-1.bmp",width =700*2,height = 700*2,units = "px",res =72*2)
df<- read.csv("十字花科 5 and 3 mismatch of multiloops2-1.csv")
...
ggplot(df,aes(x,y,fill=number))+geom_tile(width=1,height=1,color="black")+
  theme_classic()+
  scale_fill_gradientn(colours=cols,limits=c(0,1800),breaks=seq(0,1800,300),
...
library (ggplot2)
#library(tidyverse)
bmp(file ="禾本科 5 and 3 mismatch of multiloops2-1.bmp",width =700*2,height = 700*2,units = "px",res =72*2)
df<- read.csv("禾本科 5 and 3 mismatch of multiloops2-1.csv")
# df$y<-factor(df$y,levels=rev(unique(df$y)))
# df$y<-factor(df$y,levels=unique(df$y))
#df$y%>%factor()%>%fct_inorder()
#pivot_longer(df)
