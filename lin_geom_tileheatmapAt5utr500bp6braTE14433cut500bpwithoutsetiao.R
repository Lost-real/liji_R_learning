...
# library(aplot)
#library(tidyverse)
bmp(file ="At5utr500bp6bra14433merge1sortedreplacefillupcut500bpreversezhuanzhi-1.bmp",width =600*3,height = 750*3,units = "px",res =72*2)
df<- read.csv("At5utr500bp6bra14433merge1sortedreplacefillupcut500bpreversezhuanzhi-1.csv")
df$y<-factor(df$y,levels=rev(unique(df$y)))
#df$y%>%factor()%>%fct_inorder()
#pivot_longer(df)
cols <- colorRampPalette(c("black","red"))(1200)
...
ggplot(df,aes(x,y,fill=conservation))+geom_tile(width=1,height=1)+
  theme_void()+
  scale_fill_gradientn(colours=cols,limits=c(0,6),breaks=seq(0,6),
  guide=guide_colourbar(ticks=F, nbin=300,barheight=20, label=T,barwidth=4))+  
  scale_x_continuous(breaks=c(1,101,201,301,401,500),
  labels=c("-500","-400","-300","-200","-100","-1"))+
  labs(x=" ",y=" ")+
  theme(legend.background = element_rect(fill = "grey90",colour = "grey90"),
        legend.position=c(1.05,0.5),
        legend.direction="vertical",
        legend.text=element_text(colour="grey20",hjust=0.7,vjust=0.5,size=18),
		legend.margin = margin(t = 8, r = 4, b =8, l = 5,unit = "pt"),
        #plot.margin=grid::unit(c(a,b,c,d),"cm")表示图片离上下左右的距离上,右,下,左
        legend.title = element_text(color = "black", size = 20,vjust=1.8),#vjust越大越上
		plot.margin=grid::unit(c(t=0.8,r=5,b=.1,l=0.1), "cm"),
        ...
        axis.ticks.x=element_line(colour="grey20",size=1.5),
