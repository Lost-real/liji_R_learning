...
library (ggplot2)
#library(tidyverse)
bmp(file ="十字花科5UTR中5 和 3 mismatches of internal loops1-1.bmp",width =700*2,height = 700*2,units = "px",res =72*2)
df<- read.csv("十字花科5UTR中5 和 3 mismatches of internal loops1-1.csv")
...
ggplot(df,aes(x,y,fill=number))+geom_tile(width=1,height=1,color="black")+
  theme_classic()+
  scale_fill_gradientn(colours=cols,limits=c(0,400),breaks=seq(0,400,100),
                       # labels=c("0","","10^0.5","10^1.0","10^1.5","10^2.0","10^2.5","10^3.0"),
                       guide=guide_colourbar(ticks=T,ticks.colour = "black", nbin=300,barheight=10, label=T,barwidth=2))+  
...
library (ggplot2)
#library(tidyverse)
bmp(file ="禾本科5UTR中5 和 3 mismatches of internal loops1-1.bmp",width =700*2,height = 700*2,units = "px",res =72*2)
df<- read.csv("禾本科5UTR中5 和 3 mismatches of internal loops1-1.csv")
# df$y<-factor(df$y,levels=rev(unique(df$y)))
# df$y<-factor(df$y,levels=unique(df$y))
#df$y%>%factor()%>%fct_inorder()
#pivot_longer(df)
