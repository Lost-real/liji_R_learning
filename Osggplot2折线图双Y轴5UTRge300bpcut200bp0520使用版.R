...
#library(gridExtra)
bmp(file ="Os5UTR0521调整ge300bpBPPgeom_line-2.bmp",width =1000*1,height = 750*1,units = "px",res =72*2)

df1<- read.csv("Os5UTR0521调整ge300bpBPPgeom_line-2.csv")
p1<-ggplot(df1,aes(x=position,y=value,col=var))+
 # geom_line(colour="black",size=0.9)+
  ...
  scale_y_continuous("Conservation",limits = c(0,0.36),breaks=seq(0,0.36,0.09),
          sec.axis = sec_axis(~ ./3 , name = "BBP",breaks=seq(0,0.12,0.03)))+
...
bmp(file ="Os3UTR0520调整ge300bpBPPgeom_line-2.bmp",width =1000*1,height = 750*1,units = "px",res =72*2)
df2<- read.csv("Os3UTR0520调整ge300bpBPPgeom_line-2.csv")
p2<-ggplot(df2,aes(x=position,y=value,col=var))+
  ...
  #theme_light()+
  scale_y_continuous("Conservation",limits = c(0,0.48),breaks=seq(0,0.48,0.12),
  sec.axis = sec_axis(~ ./4 , name = "BBP",breaks=seq(0,0.12,0.03)))+
  guides(col=guide_legend(nrow = 1, byrow = T))+
#注意scale_color_manual(labels = c("BBP","Conservation"), values = c("green", "red"))目的
  #是自动修改图中两条line的颜色
  #scale_color_manual(labels = c("Conservation", "BPP"), values = c("red", "green"))+
