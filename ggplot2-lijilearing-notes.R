...
#tibble::glimpse用来查看数据类型,tibble是一种新的数据类型
tibble::glimpse(chic)
head(chic, 10)
version
...
#保存为csv格式
write_csv(chic,"chic03.csv")
# chic<-readr::read_csv("chic02.csv")
chic<-read_csv("chic03.csv")
#下面这个方法也是可以的chic<-readr::read_csv("chic03.csv")
#chic<-readr::read_csv("chic03.csv")
bmp(file ="123.bmp",width =1300*1,height = 600*1,units = "px",res =72*1)
g <- ggplot(chic, aes(x = date, y = temp))
#ggplot+geom_point()散点图
g + geom_point()
...
# jpeg(file ="123.jpeg",width =1300*1,height = 600*1,units = "px",res =72*1)
#firebric意思是砖红色
g + geom_point(color = "firebrick", shape = "diamond", size = 10)


g + geom_point(color = "firebrick", shape = "diamond", size = 2) +
  geom_line(color = "blue", linetype = "dotted", size = .3)+
#theme_set()可以一次性改变当前会话下的所有主题
theme_set(theme_bw())
#bw主题：black and white /黑白
theme_set(theme_bw())
#ggplot2默认的主题：theme_grey()/背景灰色的主题
theme_set(theme_grey())
#void:全空白主题(坐标轴都没有)
theme_void()
#minimal主题：去坐标轴边框的最小化主题
...
#可以一次性改变当前会话下的所有主题
theme_set(theme_bw())
#可以重置回到默认主题theme_grey()
theme_set(theme_grey())
g + geom_point(color = "firebrick")+theme_classic()
...
#字体中italic是斜体,bold.italic是加粗斜体的意思
ggplot(chic, aes(x = date, y = temp)) +
  geom_point(color = "firebrick") +
  labs(x = "Year", y = "Temperature (°F)") +
...
#expand_limits(x=0,y=0),把坐标轴扩展到(0,0)  
  expand_limits(x = 0, y = 0)



chic_high <- dplyr::filter(chic, temp > 25, o3 > 20)
#darkcyan：深青色,DarkSlateGray:深石板灰色,darkblue:深蓝色,darkgrey:深灰色
ggplot(chic_high, aes(x = temp, y = o3)) +
  geom_point(color = "DarkSlateGray") +
  labs(x = "Temperature higher than 25°F",
       y = "Ozone higher than 20 ppb") +
#coord_cartesian指定参数则控制图形特定区域放大显示
...
#coord_cartesian(clip="off")可以把图形置于最上层
...
#sienna是黄土色，褐土色
  ...
  labs(x = "Temperature (°F)", y = "Temperature (°F) + random noise") +
  xlim(c(0, 100)) + ylim(c(0, 150)) +
#coord_fixed(ratio=1/5)的含义是宽：长=1：5  
...
ggplot(chic,
  ...
  geom_point() +
  labs(x = "Year", y = "Temperature (°F)") +
#取消图例theme(legend.position = "none")
...
ggplot(chic, aes(x = date, y = o3)) +
  ...
  labs(x = "Year", y = "Ozone")+
# scale_color_discrete("Type:")更改图例的标题
