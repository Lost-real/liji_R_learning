...
# library(aplot)
#library(tidyverse)
bmp(file ="Os3utr300bp10539out1merge1sortedreplacefillupcut200bpzhuanzhi-1.bmp",width =600*3,height = 750*3,units = "px",res =72*2)
df<- read.csv("Os3utr300bp10539out1merge1sortedreplacefillupcut200bpzhuanzhi-1.csv")
...
ggplot(df,aes(x,y,fill=conservation))+geom_tile(width=1,height=1)+
  ...
  theme(legend.background = element_rect(fill = "grey90",colour = "grey90"),
		...
		plot.margin=grid::unit(c(t=0.8,r=3.4,b=.1,l=0.1), "cm"),
        ...
        axis.line.x=element_line(colour="grey20", size=1.5),
