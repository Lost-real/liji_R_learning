...
library (ggplot2)
#library(tidyverse)
bmp(file ="十字花科 the mismatch pair of hairpin loops zhuanzhi-1.bmp",width =750*2,height = 750*2,units = "px",res =72*2)
df<- read.csv("十字花科 the mismatch pair of hairpin loops zhuanzhi-1.csv")
...
ggplot(df,aes(x,y,fill=number))+geom_tile(width=1,height=1,color="black")+
  theme_bw()+
  scale_fill_gradientn(colours=cols,limits=c(0,5000),breaks=seq(0,5000,1000),
  ...
  labs(x="5' Mismatch",y="3' Mismatch")+
  theme(legend.background = element_rect(fill = "grey90",colour = "grey90"),
        legend.position=c(1.08,0.5),
        legend.direction="vertical",
        legend.text=element_text(colour="black",hjust=0.7,vjust=0.5,size=13,face="bold",family = "serif"),
...
library (ggplot2)
#library(tidyverse)
bmp(file ="禾本科 the mismatch pair of hairpin loops zhuanzhi-1.bmp",width =750*2,height = 750*2,units = "px",res =72*2)
df<- read.csv("禾本科 the mismatch pair of hairpin loops zhuanzhi-1.csv")
# df$y<-factor(df$y,levels=rev(unique(df$y)))
df$y<-factor(df$y,levels=unique(df$y))
#df$y%>%factor()%>%fct_inorder()
#pivot_longer(df)
