...
library (ggplot2)
#library(tidyverse)
bmp(file ="十字花科 5 and 3 closing pair multiloops2-1.bmp",width =750*2,height = 750*2,units = "px",res =72*2)
df<- read.csv("十字花科 5 and 3 closing pair multiloops2-1.csv")
...
ggplot(df,aes(x,y,fill=number))+geom_tile(width=1,height=1,color="black")+
  theme_classic()+
  scale_fill_gradientn(colours=cols,limits=c(0,1200),breaks=seq(0,1200,200),
  ...
  scale_y_continuous(limits=c(0,17),breaks=seq(1,16,1),
                   labels=c("G:C","U:A","A:U","C:G","G:U","U:G","A:C","C:A","U:C","A:G","C:U","G:G",
                            "U:U","C:C","A:A","G:A"))+
  scale_x_continuous(limits=c(0,17),breaks=seq(1,16,1),
                     labels=c("C:G","A:U","U:A","G:C","U:G","G:U","A:C","C:A","G:A","G:G","U:C",
                              "A:A","C:C","C:U","U:U","A:G"))+
...
library (ggplot2)
#library(tidyverse)
bmp(file ="禾本科 5 and 3 closing pair multiloops2-1.bmp",width =750*2,height = 750*2,units = "px",res =72*2)
df<- read.csv("禾本科 5 and 3 closing pair multiloops2-1.csv")
# df$y<-factor(df$y,levels=rev(unique(df$y)))
# df$y<-factor(df$y,levels=unique(df$y))
#df$y%>%factor()%>%fct_inorder()
#pivot_longer(df)
