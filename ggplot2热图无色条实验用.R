...
ggplot(df1,aes(x,y,fill=conservation))+
  geom_tile(colour="yellow",width=1,height=1)+
  theme_classic()+
...
#legend.key系列适用于离散型函数
        ...
        legend.title = element_text(color = "black", size =8,vjust=0.1),
        legend.margin = margin(t = 10, r = 1, b =8, l = 3,unit = "pt"),
       
#plot.margin=grid::unit(c(a,b,c,d),"cm")表示热图离上下左右的距离上,右,下,左
        plot.margin=grid::unit(c(t=.1,r=2,b=.1,l=0.1), "cm"),
