...
bmp(file ="202204256ATrealUTR2774ge200bpboxplot.bmp",width =600*2,height = 600*2,units = "px",res =72*2)
Data<-read.csv("202204256ATrealUTR2774ge200bpboxplot.csv", head=TRUE)
...
ggplot(Data, aes(x=Type,y=TE,fill=Type))+#根据Type进行填充，fill=Type
  ...
  ggtitle("tittle")+#ggtitle("boxplot")设置总的标题
  theme(
  ...
  axis.line.x=element_line(colour="blue",size=0.25),
  #axis.line调整x,y轴的的颜色，宽度
      axis.title.x=element_text(size=16,face="plain",color="red"),
      #axis.title调控x,y轴的文字,分别是图里面的TE和Type,
      axis.text.x = element_text(size=16,face="plain",color="purple"),
...
axis.ticks.length.x=grid::unit(0.5, "cm")
